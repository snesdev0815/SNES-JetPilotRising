...
threshold_lowest := 100
threshold_regular := 120
threshold_sprites := 130
threshold_slightly := 170
threshold_lossy := 200

gfx_bg_flags := $(verify) -optimize on -partitionTilemap on -tilethreshold $(threshold_regular) -palettes 7 -bpp 4 -mode bg
gfx_bg_slightlylossy_flags := $(verify) -optimize on -partitionTilemap on -tilethreshold $(threshold_slightly) -palettes 4 -bpp 4 -mode bg
gfx_bg_lossy_flags := $(verify) -optimize on -partitionTilemap on -tilethreshold $(threshold_lossy) -palettes 4 -bpp 4 -mode bg
gfx_bg_hilossy_flags := $(verify) -optimize on -partitionTilemap on -tilethreshold $(threshold_lossy) -palettes 7 -bpp 4 -mode bg
gfx_bg_lossy_lowcol_flags := $(verify) -optimize on -partitionTilemap on -tilethreshold $(threshold_lossy) -palettes 2 -bpp 4 -mode bg
gfx_bg_lowcol_flags := $(verify) -optimize on -partitionTilemap on -tilethreshold $(threshold_lossy) -palettes 2 -bpp 4 -mode bg
gfx_bg_static_flags := $(verify) -optimize on -partitionTilemap on  -tilethreshold $(threshold_lossy) -palettes 7 -bpp 4 -mode bg -statictiles on
gfx_bg_noop_flags := $(verify) -optimize off -partitionTilemap on  -tilethreshold $(threshold_lowest) -palettes 1 -bpp 4 -mode bg -statictiles on
gfx_status_flags := $(verify) -optimize on -tilethreshold $(threshold_lowest) -palettes 4 -bpp 2 -mode bg
gfx_status_static_flags := $(verify) -optimize on -tilethreshold $(threshold_lowest) -palettes 4 -bpp 2 -mode bg -statictiles on
gfx_level_flags := $(verify) -optimize on -collisionTileModifiable 0 -tilethreshold $(threshold_slightly) -palettes 7 -bpp 4 -mode bg -mapwidth 1024
gfx_sprite_flags := $(verify) -optimize on -tilethreshold $(threshold_sprites) -bigtilethreshold 3 -palettes 1 -bpp 4 -mode sprite -tilemultiplier 4
gfx_sprite_noop_flags := $(verify) -optimize off -tilethreshold $(threshold_sprites) -palettes 1 -bpp 4 -mode sprite -statictiles on
gfx_sprite_full_flags := $(gfx_sprite_flags) -baseframerate 0
gfx_sprite_half_flags := $(gfx_sprite_flags) -baseframerate 1
gfx_sprite_quarter_flags := $(gfx_sprite_flags) -baseframerate 2
gfx_sprite_eighth_flags := $(gfx_sprite_flags) -baseframerate 3
gfx_sprite_static_flags := $(verify) -optimize on -tilethreshold $(threshold_sprites) -palettes 1 -bpp 4 -mode sprite -statictiles on
...
gfx_sprite_ymirror_flags := $(verify) -optimize on -baseframerate 1 -palettes 2 -bpp 4 -mode sprite -tilemultiplier 4 -yMirrorTilemap on -bigtilethreshold 0
...
records += $(shell find $(datadir)/ -type f -name '*.rec')
...
datafiles := $(converted_graphics) $(converted_animations) $(converted_fonts) $(converted_palettes) $(converted_hdma_window_animations) $(converted_hdma_window_single_animations) $(converted_hdma_color_animations) $(converted_hdma_palette_animations) $(converted_songs) $(converted_samplepacks) $(spcbin) $(converted_levels) $(textfile) $(patterns) $(records) $(converted_sounds)
...
$(converted_animations): $(builddir)/%.$(spriteanimation): % | $(builddirs)
	touch $@
	$(animation_converter) $($(filter gfx_%, $(subst .,$(space), $@))_flags) -infolder $< -outfile $(subst $(space),.,$(filter-out -%,$(subst .,$(space),$@))) $(subst _,$(space),$(filter -%, $(subst .,$(space), $<)))
